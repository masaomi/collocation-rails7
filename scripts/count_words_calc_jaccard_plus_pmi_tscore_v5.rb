...
warn "# n(X∩Y) = (C + D)/2.0 = (#{search_word1_count_in_context_of_word2.keys.length} + #{search_word2_count_in_context_of_word1.keys.length})/2.0 = #{x_and_y}"
warn "# n(X∪Y) = (A + B) - n(X∩Y) = (#{search_word1_count.keys.length} + #{search_word2_count.keys.length}) - #{x_and_y} = #{x_or_y}"
...
warn "# E: #{search_word1} frequency: #{search_word1_count.keys.length} / #{total_words} = #{"%.3e" % search_word1_frequency}"
warn "# F: #{search_word2} frequency: #{search_word2_count.keys.length} / #{total_words} = #{"%.3e" % search_word2_frequency}"
warn "# G: #{search_word1} and #{search_word2} co-occurrence frequency: n(X∩Y)/(total words) = #{x_and_y} / #{total_words} = #{"%.3e" % search_word12_cooccurrence_frequency}"
...
warn "# MI score (PMI, Pointwise Mutual Information) = log2(G / (E * F)) = log2(#{"%.3e" % search_word12_cooccurrence_frequency} / (#{"%.3e" % search_word1_frequency} * #{"%.3e" % search_word2_frequency})) = #{"%.3f" % pmi2}"
warn ""

tscore = (x_and_y - (search_word1_count.keys.length * search_word2_count.keys.length)/total_words) / Math.sqrt(x_and_y)

warn "# T score = (n(X∩Y) - (A * B)/total_words) / (n(X∩Y))^(-2) = (#{x_and_y} - (#{search_word1_count.keys.length} * #{search_word2_count.keys.length})/#{total_words}) / (#{x_and_y})^(-2) = #{"%.3f" % tscore}"
warn ""
